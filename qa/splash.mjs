...
/* ---- narrow: same pinned splash as desktop, just portrait --------------- */
{
  ...
  const m = await page.evaluate(() => {
    ...
    // At rest the monitor shows the greeting (an opaque screen over the
    // hero), so the greeting is what must fit the monitor's slice. The
    // hero beneath is revealed by the push, which grows the slice to the
    // whole screen; its own layout is the page's and is not judged here.
    const line = heroStage?.querySelector(".greeting-line");
    const gb = line?.getBoundingClientRect();
    const SLACK = 8;
    const heroFits = !!gb && gb.top >= monitor.top - SLACK && gb.bottom <= monitor.bottom + SLACK && gb.left >= monitor.left - SLACK && gb.right <= monitor.right + SLACK;
    return {
      pinned: !!document.querySelector(".splash-track"),
  ...
  ok("greeting fits inside the monitor", m.heroFits, true);
  await page.close();
}
