...
export function SplashScreen({
  ...
  /* The server cannot know the reader's motion preference, so the first
     render is always the pinned splash (what the server rendered) and the
     flat one is switched to in an effect, after hydration. Choosing from
     useReducedMotion() in the render itself was a hydration mismatch. */
...
function SplashPinned({
  ...
  /* Guards the small-scale end of the hero's transform against a mismatch
     ...
     before scale reaches 1. `.hero-copy` is the actual content block (not
  ...
  useEffect(() => {
    ...
    if (!stage) return;
    const target = stage.querySelector<HTMLElement>(".hero-copy") ?? stage;
   ...
   */
  /* 0 now: the nudge above was for a hero VISIBLE on the monitor at rest.
     The monitor at rest shows the greeting, an opaque screen, and the hero
     only appears once the push is under way and the window has grown; so
     shrinking the stage to fit the hero's copy bought nothing and, on a
     phone, opened a strip of raw footage at each side of the monitor. The
     stage now always covers the window. */
  const CONTENT_WEIGHT = 0;
